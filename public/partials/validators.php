...
class dynamicpackages_Validators
{
	...
	{
		$output = false;
		global $dy_validate_quote;
		
		if(isset($dy_validate_quote))
		{
			$output = true;
		}
		else
		{
			if(isset($_POST['dy_platform']) && self::contact_details() && self::booking_details())
			{
				$output = true;
				$GLOBALS['dy_validate_quote'] = $output;
			}		
		}
		return $output;
	}
