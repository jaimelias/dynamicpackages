...
$greeting = apply_filters('dy_email_greeting', sprintf(__('Hello %s,', 'dynamicpackages'), dy_tx::request_value('first_name')));
...
$client_name = dy_tx::request_value('first_name') . ' ' . dy_tx::request_value('lastname');
$client_email = dy_tx::request_value('email');
$client_phone = dy_tx::request_value('country_calling_code') . dy_tx::request_value('phone');
...
$included = (string) dy_implode_taxo_names('package_included', __('and', 'dynamicpackages'), '✅');
$label_included = __('Included', 'dynamicpackages');
$not_included = (string) dy_implode_taxo_names('package_not_included', __('or', 'dynamicpackages'), '❌');
...
$whatsapp = (get_option('dy_whatsapp')) ? '<a style="border: 16px solid #25d366; text-align: center; background-color: #25d366; color: #fff; font-size: 18px; line-height: 18px; display: block; width: 100%; box-sizing: border-box; text-decoration: none; font-weight: 900;" href="'.esc_url($whatsapp_url).'">'.__('Whatsapp Advisory', 'dynamicpackages').'</a>' : '';
