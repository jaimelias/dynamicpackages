...
class Dynamicpackages_LD_JSON
{
	...
	public function website_ld_json($arr = []) {
		...
		if (empty($package_main)) {
			$package_main = get_option('page_on_front');
		}
		$package_main = (int) $package_main;

		
		if(function_exists('pll_get_post') && $current_language !== $default_language)
		{	
	...
	public function event_arr() : array {
		$output = [];
		$the_id = get_dy_id();
		$cache_key = 'event_arr_' . $the_id;
		...
		if(!empty($package_start_address) && !empty($package_start_hour))
		{
			...
			if($from >= 0 && $to > $from)
			{
				...
				$disabled_range = dy_utilities::get_disabled_range($the_id);
				$week_days = dy_utilities::get_week_days_list($the_id);
