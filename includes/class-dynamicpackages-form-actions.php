...
class Dynamicpackages_Actions{
	...
	public function subject()
	{
		if(dy_validators::validate_quote())
		{
			$output = sprintf(__('%s, %s has sent you an estimate for %s%s - %s', 'dynamicpackages'), sanitize_text_field($_POST['first_name']), get_bloginfo('name'), currency_symbol(), money_format(dy_utilities::total()), sanitize_text_field($_POST['title']));			
		}
		else
		{
			global $post;
