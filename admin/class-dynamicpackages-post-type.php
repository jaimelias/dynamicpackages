...
class Dynamicpackages_Post_Types
{
	...
	function __construct()
	{
		...
		add_action('init', [$this, 'package_post_type']);
	...
	public function package_post_type() {
		...
		$args = [
			...
			'labels' => $labels,
			'supports' => [ 'title', 'editor', 'thumbnail', 'page-attributes', 'excerpt', 'comments'],
			'hierarchical' => true,
			'public' => true,
			'show_ui' => true,
			'show_in_menu' => true,
	...
	public function register_taxonomies()
	{
		foreach ($this->get_taxonomies_arr() as $taxonomy => $config) {
			...
			$labels = array_merge($config, [
				...
				'parent_item_colon' => sprintf(__('Parent %s', 'dynamicpackages'), $singular),
				'edit_item'         => sprintf(__('Edit %s', 'dynamicpackages'), $singular),
				'update_item'       => sprintf(__('Update %s', 'dynamicpackages'), $singular),
